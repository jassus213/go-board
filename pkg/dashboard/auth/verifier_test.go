...
func TestJWTVerifier_Verify(t *testing.T) {
	...
	// Helper to create token with wrong secret
	...
	tests := []struct {
		...
		},
		{
			...
			wantUserID: "",
			wantErr:    ErrTokenExpired, // Updated to match verifier.go
		},
		{
			...
			wantUserID: "",
			wantErr:    ErrMissingUserID, // Updated to match verifier.go
		},
		{
		...
		},
		{
			...
			token: createToken(jwt.MapClaims{
				"user_id": "user999",
			}, jwt.SigningMethodHS512), // Different method
			wantUserID: "",
			wantErr:    ErrInvalidToken,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantErr != nil {
				assert.Error(t, err)
				// For wrong_signing_method, JWT lib deeply wraps the error
				// so we check both ErrInvalidToken and that message contains "signing method"
