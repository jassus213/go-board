...
func (r *DashboardRedisRepository) AddMembersBatch(ctx context.Context, dashboard string, members []domain.DashboardRecord) error {
	if len(members) == 0 {
		return nil
	}
...
func (r *DashboardRedisRepository) GetTopMembers(ctx context.Context, dashboard string, top int64) ([]domain.DashboardRecord, error) {
	if top <= 0 {
		return nil, nil
	}
	...
	records := make([]domain.DashboardRecord, len(result))
	for i, memberData := range result {
		records[i] = domain.DashboardRecord{
...
func (r *DashboardRedisRepository) IncrementMembersBatch(ctx context.Context, dashboard string, increments []domain.DashboardRecord) error {
	if len(increments) == 0 {
		return nil
	}
